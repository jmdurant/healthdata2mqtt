...
print("""
===============================================
Export 2 Garmin Connect v3.0 (s400_export.py)
===============================================
""")
...
selected_user = None
for user in users:
    if user.min_weight <= s400_weight <= user.max_weight:
        selected_user = user
        break

# Calcuating body metrics
if selected_user is not None and 'email@email.com' not in selected_user.email:
    lib = Xiaomi_Scale_Body_Metrics.bodyMetrics(s400_weight, selected_user.height, selected_user.age, selected_user.sex, int(s400_impedance))
    bmi = lib.getBMI()
    ...
    print(f"S400 * Calculated data: {formatted_time};{s400_weight:.1f};{bmi:.1f};{percent_fat:.1f};{muscle_mass:.1f};{bone_mass:.1f};{percent_hydration:.1f};{physique_rating:.0f};{visceral_fat_rating:.0f};{metabolic_age:.0f};{basal_met:.0f};{lib.getLBMCoefficient():.1f};{lib.getIdealWeight():.1f};{lib.getFatMassToIdeal()};{lib.getProteinPercentage():.1f};{s400_impedance:.0f};{selected_user.email};{dt.now().strftime('%d.%m.%Y;%H:%M')}")
    ...
    with open(path + '/user/' + selected_user.email, 'r') as token_file:
        ...
        garmin.add_body_composition(dt.fromtimestamp(s400_datetime).isoformat(),weight=s400_weight,bmi=bmi,percent_fat=percent_fat,muscle_mass=muscle_mass,bone_mass=bone_mass,percent_hydration=percent_hydration,physique_rating=physique_rating,visceral_fat_rating=visceral_fat_rating,metabolic_age=metabolic_age,basal_met=basal_met)
        print("S400 * Upload status: OK")
else:

    # Print to temp.log file
    print(f"S400 * Import data: {s400_datetime};{s400_weight:.1f};{s400_impedance:.0f}")
